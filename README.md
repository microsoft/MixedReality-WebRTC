...
The MixedReality-WebRTC project consumes its input dependencies as NuGet packages for simplicity, because those are complex to build and take time. The steps below are recommended for most users who need to modify the source code of MixedReality-WebRTC without the need to change the input dependencies.

_Note_ - Although this is **strongly discouraged** for most users due to its complexity, a detailed step-by-step tutorial on building from source including the dependencies is also available, see [Building from sources](building.md).
