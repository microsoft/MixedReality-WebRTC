...
function Initialize-BuildEnvironment
{
    ...
    if (!($env:PATH.Contains($depotToolsPath)))
    {
        # Be sure to append, not prepend. depot_tools/ contains some git version,
        # and prepending will make that version the default one, which may confuse
        # some users as e.g. on Windows it won't have the credential manager set up
        # like the one the user had installed already.
